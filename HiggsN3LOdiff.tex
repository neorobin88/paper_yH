...
\begin{align}
  ...
  \label{eq:hadXS_diff}
\end{align}
where $\rd\hat{\sigma}_{ab}$ is the differential partonic cross section and $f_{c/h}$ corresponds to the distribution of a parton $c$ in a hadron $h$.
% and $\bOmega$ encodes the degrees of freedom associated with the decay products of $F$.
Since $F$ is colourless, the LO partonic cross section can be either initiated by $q\bar{q}^{\prime}$ annihilation, as in the case of the Drell--Yan process, or by gluon--gluon fusion, as in the case of Higgs boson production.
...
In order to explain the basic idea  of the subtraction formalism, we first notice that at LO, the transverse momentum $\qt$ of the final state system $F$ is identically zero. Therefore, as long as $\qt \neq 0$, the \N{n}\LO QCD contributions (with $n=1,2,3$) are given by the \N{n-1}\LO QCD contributions to the $F$+jet(s) final state. Consequently, if $\qt \neq 0$ we have:
\begin{equation}
...
\begin{align}
  ...
  \left[ 
    \frac{\rd\sigma^{F+\jets}_{\N{n-1}\LO}}{\rd\qt^2\rd M^2\rd Y} -
    \frac{\rd\sigma^{F\;\text{CT}}_{\N{n}\LO}(\qt)}{\rd\qt^2\rd M^2\rd Y}
  \right]
  \label{eq:master}
\end{align}
...
\begin{align}
  ...
  \label{eq:CT}
\end{align}
where we note that the dependence of the function $\Sigma^{F\;\text{CT}}_{\N{n}\LO}(\qt)$ on the transverse momentum $\qt$ is \emph{not} kinematical but constitutes a dynamical parameter in this context.
As such, the kinematic constraint $\delta(\qt^2)$ appearing inside the LO cross section~\eqref{eq:LOpartXS_diff} must not act on $\qt$ within the function $\Sigma^{F\;\text{CT}}$.

The symbol ``$\otimes$'' appearing in Eqs.~\eqref{eq:master} and \eqref{eq:CT} denotes convolutions over the momentum fractions and the flavour indices of the incoming partons and is explicitly defined as
\begin{align}
  \cG(\dots) \otimes \frac{\rd\sigma^{F}(p_1, p_2)}{\rd\qt^2\rd M^2\rd Y} 
  &\equiv
  \int_0^1\rd x_1 \int_0^1\rd x_2 \;
  \frac{\rd\hat{\sigma}^{F}_{ab}(x_1 p_1, x_2 p_2)}{\rd\qt^2\rd M^2\rd Y}
  \nonumber\\&\quad
...
\end{align}
\AH{We can also directly write it for the LO cross section (and thus use $c\bar{c}$) by inserting Eq.~\eqref{eq:LOpartXS_diff}:}
\begin{align}
  \cG(\dots) \otimes \frac{\rd\sigma^{F}_{\LO}(p_1, p_2)}{\rd\qt^2\rd M^2\rd Y} 
  &\equiv
  \delta(\qt^2)\;\frac{1}{s}\;
  \rd\hat{\sigma}^{F}_{\LO;c\bar{c}}(x_1 p_1, x_2 p_2)
  \nonumber\\&\quad
  \times 
  \int_{x_1}^1 \frac{\rd z_1}{z_1} \int_{x_2}^1 \frac{\rd z_2}{z_2} \; 
  \cG_{c\bar{c} \gets ab}(\ldots; z_1, z_2) \;
  f_{a/h_1}\left(\frac{x_1}{z_1},\muf\right) \; f_{b/h_2}\left(\frac{x_2}{z_2},\muf\right) 
\end{align}
\AH{there is no $\delta(M^2/\hat{s}-z_1 z_2)\sim\delta(M^2-x_1 x_2 z_1 z_2 s)$ here because the correct constraint is $\delta(M^2-x_1 x_2 s)$ without the $z_i$'s that was used to kill one of the $x_i$ integrations. The splitting is $p_i \to \frac{x_i}{z_i} p_i \to z_i \frac{x_i}{z_i} p_i = x_i p_i$ which I tried to make a bit clearer with the arguments of $\hat{\sigma}(x_i p_i)$. In the NNLOJET language: This convolution is written in the \texttt{imethod=2} convention where the partonic kinematics is kept fixed and instead the arguments of the PDFs are modified. So for any $z_i$ value $M^2=x_1 x_2 s$ and $Y=\ln(x_1/x_2)/2$. This is why Eq.~\eqref{reslean} is now actually correct \emph{differentially} because both l.h.s.\ \& r.h.s.\ correspond to the same kinematics ($M$, $Y$). This was not true in the previous case where we used $\cG(z)$ instead of $\cG(z_1,z_2)$, where the equation only was valid after rapidity integration.}

The functions $\cH^{F}_{\N{n}\LO}$ and $\Sigma^{F}_{\N{n}\LO}(\qt)$ correspond to the $n$-th order truncation of the perturbative series in $\as$ of the functions
{\color{black!30}
\beeq
...
\begin{align}
  ...
  \label{Hstexpand}
\end{align}
where the labels $a$ and $b$ stands for the partonic channels of the N$^{n}$LO correction to the Born cross section. 
Notice that at LO, the only available configuration is $a=c$ and $b=\bar{c}$, where $c\bar{c}$ is (are) the partonic channel(s) through which the LO cross section is initiated\AH{this sentence is repitition}.
The function $ \Sigma^{F}(\qt^{2}/M^{2})$ embodies all the logarithmic terms that are divergent in the limit $\qt \rightarrow 0$ reproducing the singular behaviour of  $\rd\sigma^{F+\jets}$ in the small-$\qt$ limit. The definition of the counterterm is free of terms proportional to $\delta(\qt^{2})$ which are all absorbed in the perturbative factor $\mathcal{H}^{F}$. The hard coefficient function $\mathcal{H}^{F}_{\rm N^{n}LO}$ thus encodes all the IR finite terms of the $n$-loop contributions.
...
\begin{align}
  \Bigl( \Sigma^F_{\N{n}\LO}(\qt)  + \cH^F_{\N{n}\LO} \Bigr) \otimes 
  \frac{\rd\sigma^{F}_{\LO}}{\rd\qt^2\rd M^2\rd Y} 
  &=
  ...
  \label{reslean}
\end{align}
where  $b_0=2 \mathrm{e}^{-\gamma_E}$ ($\gamma_E=0.5772...$  is the Euler--Mascheroni constant).
% , and the kinematical variables $x_1$ and $x_2$ are 
% \begin{equation}
% \label{xo}
% x_1= \f{M}{\sqrt s} \;e^{+y} \;\;, \quad \quad
% x_2=\f{M}{\sqrt s} \;e^{-y} \;\;.
% \end{equation}
The right-hand side of Eq.~(\ref{reslean}) involves the Fourier transformation with respect to the \emph{impact parameter} $b$ and two convolutions over the longitudinal-momentum fractions $z_1$ and $z_2$. 
{\color{black!30}
In the left-hand side of Eq.~\eqref{reslean}, we omit the full scale dependence of these coefficients, which is however, fully specified in Eqs.~\eqref{eq:sigexpansion} and \eqref{Hstexpand}\AH{dropped above}.
} 
{\color{black!30}While the coefficient functions $\Sigma^{F}$ and $\cH^{F}$ in the left-hand side of Eq.~\eqref{reslean} depend on the longitudinal-momentum fractions $z_1$ and $z_2$, the corresponding rapidity integrated coefficient functions (which depend only on $z$) are defined in Eqs.~\eqref{eq:sigexpansion} and \eqref{Hstexpand}.
The symbolic factor $\rd\hat{\sigma}^{F;(0)}_{c\bar{c}}$ for the partonic Born cross section $\hat{\sigma}^{F;(0)}_{c\bar{c}}$ denotes
\beeq
...
\AH{this block is hopefully all properly defined in the new equations above}
...
\eeeq
where 
$H_{g}^{F=H}$ is the hard--virtual function, and $C_{g \,a}$ and $G_{g \,a}$ are 
...
\AH{we completely removed the azimuthal dependence from the start, maybe put this in a footnote?}
}
...
Explicit expressions for the fixed-order coefficients are collected in Appendix~\ref{app:fixed-order}, in particular, we also give the $B_3^H$ coefficient in the hard scheme as needed for the $\qt$ subtraction formalism described in this section.
An obstacle in applying the $\qt$ subtraction formalism at \N3\LO are the missing ingredients within $\cH^{F=H;(3)}$ and one of the main objectives in this work is the numerical extraction of these parts that are not yet known analytically.
To this end, we first quote the fixed-order coefficient functions $\cH^{F=H;(n)}$ up to $n=3$, where we restrict ourselves to the case $\muf=\mur=M$ in order to keep the size of the expressions compact and not to obfuscate from the main goal, which is the identification of the unknown parts at \N3\LO.
...
\begin{equation}
\label{HCCGG}
{\cal H}^H_{gg\gets ab}(z;\as) \equiv H_g^H(\as) \!\int_0^1\!\rd z_1 \int_0^1\!\rd z_2
\,\delta(z - z_1z_2)
...
Notice two main differences between Eqs.~\eqref{whathlean} and \eqref{HCCGG}. The first difference is related to the fact that the function  ${\cal H}^H$ depends on the energy fraction $z$, due to the convolution integral (over the momentum fractions $z_1$ and $z_2$) in the right-hand side of Eq.~\eqref{HCCGG}. This convolution kinematically arises by considering the integration of Eq.~(\ref{reslean}) over the rapidity $y$ of the Higgs boson. The second difference concerns the scale of $\as$: in the functions $H^H_g(\as), C(\as)$ and $G(\as)$ on the right-hand side of Eq.~\eqref{HCCGG}, the strong coupling constant is evaluated at a common scale, which is not explicitly denoted in the above equation. Owing to this feature, the process-dependent function ${\cal H}^H_{gg\gets ab}$ is unambiguously defined
...
\begin{align}
\label{H1}
{\cal H}^{H;(1)}_{gg\gets ab}(z)&=\delta_{g\,a} \,\delta_{g\,b} \,\delta(1-z) 
\,H^{H;(1)}_g+
\delta_{g\,a} \,C^{(1)}_{g\,b}(z)+\delta_{g\,b} \,C^{(1)}_{g\,a}(z) \;\; , \\
\label{H2}
{\cal H}^{H;(2)}_{gg\gets ab}(z)&=\delta_{g\,a} \,\delta_{g\,b} \,\delta(1-z)
\,H^{H;(2)}_g
...
At \N3\LO, the universal relation between $H^{F;(3)}_{c}$ and the third-order virtual correction to the partonic subprocess $c{\bar c}\to F$ presents one missing ingredient: a \emph{single} coefficient (of \emph{soft} origin) belonging to the finite part of the structure of the IR singularities contained in the
...
The missing terms in Eq.~\eqref{H3approx}, concerning the final and full expression of $H^{H;(3)}_g$, have a \emph{soft} origin. Following the notation of Ref.~\cite{Catani:2013tia}, all the third-order terms in the right-hand side of Eq.~(81) of Ref.~\cite{Catani:2013tia} are denoted by $\delta^{\qt}_{(2)}$. This allows to perform a subsequent decomposition for the third-order hard--virtual coefficient defined in Eq.~\eqref{H3approx}
\beeq
\label{H3deltaqT2}
H^{H;(3)}_g  \equiv  \widetilde{H}^{H;(3)}_{g} + \big[H^{H;(3)}_{g}\big]_{(\delta^{\qt}_{(2)})} \;.
\eeeq
As a consequence, the only missing ingredients to ${\cal H}^{F \,;(3)}$ are the functions $G^{(2)}_{g\,a}(z)$, $C^{(3)}_{g\,a}(z)$ and $ \big[H^{H;(3)}_{g}\big]_{(\delta^{\qt}_{(2)})}$. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We start this section with some observations related to the hard-scattering function ${\cal H}_{c{\bar c} \gets ab}^{F}$. This function is resummation-scheme independent, but it depends on the specific hard-scattering subprocess $c + {\bar c} \to F$. The coefficients ${\cal H}_{c{\bar c} \gets ab}^{F \,;(n)}$ of the perturbative expansion in Eq.~\eqref{Hstexpand} can be determined by performing a perturbative calculation of the $\qt$ distribution in the limit $\qt \to 0$. In the right-hand side of Eq.~\eqref{reslean},  the function ${\cal H}^{F}$ controls the strict perturbative normalization of the corresponding total cross section (i.e.\ the integral of the total $\qt$ distribution). This unitarity-related property can be exploited to determine the coefficients ${\cal H}_{c{\bar c} \gets ab}^{F \,;(n)}$ from the perturbative calculation of the total cross section.  At the partonic level, the integral of the total $\qt$ distribution in Eq.~\eqref{eq:master} must be equal to the total cross section ${\hat \sigma}_{F \,ab}^{{\rm {tot}}}$,
\begin{equation}
\label{restotp}
...
\as(\mu_R^2),\mu_R^2,\mu_F^2) 
 \;.
\end{equation}
Since the the hard-scattering function ${\cal H}_{c{\bar c} \gets ab}^{F}$ is simply proportional to $\delta(\qt^{2})$, we evaluate the $\qt$ spectrum on right-hand side of Eq.~\eqref{eq:master} according to the following decomposition
\beeq
\label{sigtotrel} 
...
\eeeq
where $d{\hat \sigma}_{F \,ab}^{(\rm fin.)}$ is directly related to the quantity in square bracket in the right-hand side of Eq.~\eqref{eq:master}
\beeq
\label{sigfin}
...
\right\} . \nn
\eeeq
At LO, where only the Born subprocess is available, $\Bigl[ \f{\rd{\hat \sigma}_{F \,ab}^{(\rm fin.)}}{\rd \qt^2} \Bigr]_{\rm LO}$ is zero by definition. Notice that in Eqs.~\eqref{h3nnlo} and \eqref{hn} the full  ${\cal H}_{c{\bar c} \gets ab}^{F \,;(n)}$ coefficient is used, i.e., the scale independent terms as well as the terms that we omitted in Eq.~\eqref{HCCGG} (and therefore in Eqs.~\eqref{H1}, \eqref{H2} and \eqref{H3}) embody the scale dependence through the ratios $M^{2}/\mu^{2}_{F}$ and $M^{2}/\mu^{2}_{R}$.
...
The general structure of the coefficient ${\cal H}^{F \,;(3)}$ (which is proportional to $\delta(\qt^{2})$) is not known in analytic form for any hard-scattering process. Nonetheless, within the $\qt$ subtraction  formalism, ${\cal H}^{F \,;(3)}$ can be reliably approximated for any hard-scattering process whose corresponding total cross section is known at \N3\LO. 
...
{The total cross section for Higgs boson production $\sigma_{H}^{{\rm {tot}}}$ at the LHC ($\sqrt{s}=13$~TeV). Results for NNLO and \N3\LO cross sections for three different scales $\mu=M_{H}/2$ (central scale), $\mu=M_{H}$ and $\mu=M_{H}/4$. The column \lq{}\lq{}Exact\rq{}\rq{} contains the results of Ref.~\cite{Mistlberger:2018etf} computed with the numerical code of Ref.~\cite{Dulat:2018rbf} as detailed in the text. The results with the $\qt$ subtraction method are obtained using three different values of $\qtcut$ (1,2 and 3~GeV), and their uncertainties are calculated with the customary propagation of statistical errors. The last column shows $ \sigma_{H}^{{\rm {tot}}}$ obtained with the $\qt$ subtraction method and using $C_{N3}=0$ at \N3\LO. The values of $ \sigma_{H}^{{\rm {tot}}}$ reported in this Table are shown in Fig.~\ref{fig:totXsecN3LO}. The NNLO cross sections computed with the $\qt$ subtraction method are obtained using $\qtcut=1$~GeV, i.e.\ the variation of this parameter in the \N3\LO cross section is considered at \N3\LO-only.
}
}
\renewcommand{\arraystretch}{1}
\end{table}
