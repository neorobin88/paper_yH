...
Using Eqs.~\eqref{reslean} and \eqref{whathlean}, then, after integration over $b$ and dropping the renormalisation group predictable terms that are produced by evolving $\as$ to a common scale (i.e. setting $\muF=\muR=M$), we obtain 
\begin{align}
  ...
  \label{HCCGG}
\end{align}
Note that in the literature, it is often the rapidity-integrated variant $\cH^H_{gg\gets ab}(z)$ that is quoted and which is related to $\cH^H_{gg\gets ab}(z_1,z_2)$ via the convolution
\begin{align}
  \cH^H_{gg\gets ab}(z)
  &=
