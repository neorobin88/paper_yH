...
The kinematics of the Born cross section are further strongly contrained such that
\begin{align}
  \frac{\rd\hat{\sigma}^{F}_{\LO;c\bar{c}}(x_1 p_1, x_2 p_2)}{\rd\qt^2\rd M^2\rd Y\rd\bOmega}
  &=
  \delta(\qt^2) \;
  \delta\bigl(M^2-x_1 x_2 s\bigr)
  \delta\bigl(Y-\ln(x_1/x_2)/2\bigr)
  \frac{\rd\hat{\sigma}^{F}_{\LO;c\bar{c}}(x_1 p_1, x_2 p_2)}{\rd\bOmega\rvert_{\qt=0}} \; 
  \nn\\
  &=
  \delta(\qt^2) \;
...
\begin{equation}
  \left. \frac{\rd\sigma^{F}_{\N{n}\LO}}{\rd\qt^2\rd M^2\rd Y\rd\bOmega} \right\rvert_{\qt>0}
  \equiv 
  \frac{\rd\sigma^{F+\jets}_{\N{n-1}\LO}}{\rd\qt^2\rd M^2\rd Y\rd\bOmega} \;,
  \label{Eq:first}
\end{equation}
\AH{define \obs}
...
\end{equation}
\begin{align}
  \frac{\rd\sigma^{F}_{\N{n}\LO}}{\rd\qt^2\rd M^2\rd Y\rd\bOmega}
  &=
  \cH^F_{\N{n}\LO} \otimes
  \frac{\rd\sigma^{F}_{\LO}}{\rd\qt^2\rd M^2\rd Y\rd\bOmega} +
  \left[ 
    \frac{\rd\sigma^{F+\jets}_{\N{n-1}\LO}}{\rd\qt^2\rd M^2\rd Y\rd\bOmega} -
    \frac{\rd\sigma^{F\;\text{CT}}_{\N{n}\LO}(\qt)}{\rd\qt^2\rd M^2\rd Y\rd\bOmega}
  \right]
  \label{eq:master}
\end{align}
...
\begin{align}
  \cG \otimes \frac{\rd\sigma^{F}(p_1, p_2)}{\rd\qt^2\rd M^2\rd Y\rd\bOmega} 
  &\equiv
  \int_0^1\rd x_1 \int_0^1\rd x_2 \;
  \frac{\rd\hat{\sigma}^{F}_{ab}(x_1 p_1, x_2 p_2)}{\rd\qt^2\rd M^2\rd Y\rd\bOmega}
  \nonumber\\&\quad
...
\begin{align}
  ...
  \frac{\rd\sigma^{F}_{\LO}}{\rd\qt^2\rd M^2\rd Y\rd\bOmega} 
  \nn\\&=
  \frac{M^2}{s}\int\rd b\;\frac{b}{2}\;J_0(b\qt)
  \frac{\rd\hat{\sigma}^{F}_{\LO;c\bar{c}}(x_1 p_1, x_2 p_2)}{M^2\,\rd\bOmega\rvert_{\qt=0}} \; 
  S_c(M,b) \; \left[ H^{F} C_1 C_2 \right]_{c\bar{c};ab} \;
...
% x_2=\f{M}{\sqrt s} \;e^{-y} \;\;.
% \end{equation}
The right-hand side of Eq.~(\ref{reslean}) involves the Fourier transformation with respect to the \emph{impact parameter} $b$ and two convolutions over the 
longitudinal-momentum fractions $z_1$ and $z_2$. In the left-hand side of Eq.~\eqref{reslean}, we omit the full scale dependence of these coefficients, which is however, fully specified in Eqs.~\eqref{eq:sigexpansion} and \eqref{Hstexpand}\AH{maybe drop it above?}. 
...
Explicit expressions for the fixed-order coefficients are reported in Appendix~\ref{app:fixed-order}.

One of the main objectives in this work is the numerical extraction of the unknown ingredients that enter the hard coefficient function at the third order, $\cH^{F=H;(3)}$, which will be discussed in detail in the following section.
The hard coefficient function is explicitly given by 
...
where we have set $\mur=\muf=M$ to reduce the size of the expressions...
\AH{This is where Eqs.(26)--(29) will be moved.}
